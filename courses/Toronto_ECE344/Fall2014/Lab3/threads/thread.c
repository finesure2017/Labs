...
struct lock
{
	int available; // 1 => available, 0=> Used
	struct wait_queue *queue; // a wait queue associated with this lock  
	int tid; // the current thread holding this lock 
	int enabled; // previous state of signal
};
...
struct lock * lock_create()
{
	...
	lock->available = 1; // 
	lock->tid = 0; // value doesn't matter if it is available 
	lock->enabled = 0; 
...
void lock_acquire(struct lock *lock)
{
	...
	lock->tid = headRunQueue->tid; 
	interrupts_set(lock->enabled);
	return; 
}
...
{
	assert(lock != NULL);
	// Make sure lock is unavailable
	if(!lock->available)
	{
		// Make sure lock belongs to current running thread
		if(lock->tid == headRunQueue->tid)
		{
			// Make lock available
			lock->available = 1; 
			// Wake up all threads 
...
struct cv
{
	struct wait_queue *queue; // a wait queue associated with this condition variable 
};
...
void cv_wait(struct cv *cv, struct lock *lock)
{
	...
	assert(lock->tid == headRunQueue->tid); 

//unintr_printf("CVQUEUE\n");
//printWaitQueue(cv->queue); 
//unintr_printf("LOCKQUEUE\n");
//printWaitQueue(lock->queue); 
