...
// Only make changes to this file for Lab2 and Lab3 
...
{	
	...
	ucontext_t context; // to handle context switching, set as non pointer so allocated at init , note: Every context has it's own signal sets!!!!! 
...
struct thread* headRunQueue;   // the head of the runQueue
struct thread* headDeletedQueue;  // the head of the deletedQueue that are waiting to be deleted 
// Note: Head of waiting queue is implemented below for LAB3
// TODO: Head of Blocked queue 


// An array to make sure no 2 threads have the same tid
...
Tid thread_create(void (*fn) (void *), void *parg)
{
	...
	if (enabled)
	{	
		sigaddset(&(newThread->context.uc_sigmask), SIG_TYPE); 
	}
	// If signals were not enabled, exclude it from signal mask 
	else
	{
		sigdelset(&(newThread->context.uc_sigmask), SIG_TYPE); 			
	}
	// TODO: Handle alignment here 
	getcontext(&(newThread->context)); // save the context for newThread 
	...
	// Restore signal state
	enabled = sigismember(&(newThread->context.uc_sigmask), SIG_TYPE); 	
	interrupts_set(enabled); 
	return id; 
}
...
Tid thread_yield(Tid want_tid)
{
	...
	// If signals were enabled, include it into signal mask of current thread 
	if (enabled)
	{	
		sigaddset(&(headRunQueue->context.uc_sigmask), SIG_TYPE); 
	}
	// If signals were not enabled, exclude it from signal mask of current thread 
	else
	{
		sigdelset(&(headRunQueue->context.uc_sigmask), SIG_TYPE); 			
	}
	
	int flag = 0; // to make sure when context switching, don't get into infinite loop 
	...
	if (want_tid == THREAD_ANY)
	{
		...
		else 
		{
			...
			// curr points to current thread's head which is now the last in the list
			// headRunQueue points to new thread's head
			getcontext(&(curr->context)); // this is where the old thread should return to. 
			if (!flag)
			{
				flag = 1; 
				setcontext(&(headRunQueue->context));  // go to new thread's context
			}
			else
			{ // will enter here after returning from old thread 
				flag = 0; 
				// TODO: Get current signal from context, put it into enabled
				enabled = sigismember(&(curr->context.uc_sigmask), SIG_TYPE); 
					// returns 1 if SIG_TYPE is in signal mask
					// returns 0 if SIG_TYPE wasn't in signal mask 
				// Restore signal state
				interrupts_set(enabled);
				return id; 
			}
	...
	{
		int id = headRunQueue->tid; 
		getcontext(&(headRunQueue->context)); // store state to return to next time 
		if (!flag)
		{
			flag = 1; 
		...
		{
			flag = 0; 
			enabled = sigismember(&(headRunQueue->context.uc_sigmask), SIG_TYPE); 
				// returns 1 if SIG_TYPE is in signal mask
				// returns 0 if SIG_TYPE wasn't in signal mask 
			// Restore signal state
			interrupts_set(enabled);
