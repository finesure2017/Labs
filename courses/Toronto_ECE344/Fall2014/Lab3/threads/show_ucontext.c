...
int main(int argc, char **argv)
{
	...
	if (setcontext_called == 1) {
		...
		// Yes, after executing bottom, since getContext returns twice, once immediately and once after setcontext
		// we get here after setcontext
		show_interrupt();
		exit(0);
	}
	...
	// rip = register instruction pointer = points to current instruction 
	...
	setcontext_called = 1; // to make sure we dont end up with infinite loop 
	call_setcontext(&mycontext);
	/* QUESTION: why does the program not fail at the assert below? */
	// Cause on successful, setcontext() doesn't return but returns at getcontext() 
	assert(0);
}
