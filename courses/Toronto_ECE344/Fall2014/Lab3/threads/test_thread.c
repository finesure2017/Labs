...
test_basic()
{
	...
	/* create a thread */
	ret = thread_create((void (*)(void *))hello, "hello from first thread"); // create new thread and it's return value will be ret. It is 1 
	assert(thread_ret_ok(ret));			
	ret2 = thread_yield(ret);		// yield to new thread, thus, old thread, returns to new thread as 1, new thread finshes and returns to here as 1 
	assert(ret2 == ret);	
	...
	for (ii = 0; ii < NTHREADS; ii++) {
		...
		assert(ret > 0);
		child[ii] = thread_create((void (*)(void *))hello, msg[ii]); // get values of all create tid
		assert(thread_ret_ok(child[ii]));
	}
	for (ii = 0; ii < NTHREADS; ii++) {
		ret = thread_yield(child[ii]);	// yield to all created tid 
		assert(ret == child[ii]);
	}
	...
	assert(ret == ret2);
	for (ii = 0; ii < NTHREADS; ii++) {
		ret = thread_exit(child[ii]);	// destroy all threads created 
		assert(ret == child[ii]);
	}
...
test_preemptive()
{
	...
	unintr_printf("this test will take %d seconds\n", DURATION / 1000000); // print 60 seconds
	gettimeofday(&pstart, NULL); // save current time of day 
	/* spin for sometime, so you see the interrupt handler output */
	spin(SIG_INTERVAL * 5);
	interrupts_quiet(); // turn off interrupts 
	...
	for (ii = 0; ii < NPOTATO; ii++) {
		potato_tids[ii] =
			thread_create((void (*)(void *))do_potato, (void *)ii); // create all threads to do potato 
		assert(thread_ret_ok(potato_tids[ii]));
	}

	spin(DURATION); // wait for a while 

	unintr_printf("cleaning hot potato\n");

	for (ii = 0; ii < NPOTATO; ii++) {
		ret = thread_exit(THREAD_ANY); // now remove all thread one by one 
		assert(thread_ret_ok(ret));
	}
...
do_potato(int num)
{
	...
	// Print thread number that made it to function do potato 
...
try_move_potato(int num, int pass)
{
	...
	struct timeval pend, pdiff;
		...
		// pass in an integer called potato_lock 
	err = __sync_bool_compare_and_swap(&potato_lock, 0, 1); // if potato_lock is 0, store 1 into it 
...
test_wakeup(int all)
{
	...
	queue = wait_queue_create(); // create the wait queue after disabling interrupts 
	assert(queue);
	...
	/* create all threads */
	for (ii = 0; ii < NTHREADS; ii++) {
	unintr_printf("Before Creating Thread %d\n", ii);

		child[ii] = thread_create((void (*)(void *))test_wakeup_thread,
					  (void *)ii);
	unintr_printf("After Creating Thread\n");

		assert(thread_ret_ok(child[ii]));
	}
	unintr_printf("Done Creating Thread\n");
...
out:
	...
	unintr_printf("Returned for ret is: %d and all is %d\n", ret, all );

		assert(ret >= 0);
		assert(all ? ret <= NTHREADS : ret <= 1);
	}
	unintr_printf("After WHILE LOOP \n");
...
static void test_lock_thread(unsigned long num)
{
	int i, j;
	// Loop 10 times in i, 1000 times in j 
	for (i = 0; i < LOOPS; i++) {
		for (j = 0; j < NLOCKLOOPS; j++) {
			lock_acquire(testlock); // acquire the test lock 
			testval1 = num;
			testval2 = num * num;
			testval3 = num % 3;
...
test_lock()
{
	...
	testlock = lock_create(); // create a lock 
	done = 0;
